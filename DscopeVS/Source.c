...
int main(void)
{
	...
	/*		int16_t aa = 1;
			...
			/*AO1 FIFO*/
			uint32_t AO1rate = 1; //in us
			size_t size = FIFODEPTH1;
			int16_t AOFIFOdata[FIFODEPTH1]; //the analog output takes a signed 16-bit int
			AOFIFOdata[0] = _I16_MAX;
			NiFpga_MergeStatus(&status, NiFpga_WriteU32(session, NiFpga_FPGA_ControlU32_AO1LoopPeriodus, AO1rate)); //rate
			NiFpga_MergeStatus(&status, NiFpga_WriteFifoU32(session, NiFpga_FPGA_HostToTargetFifoI16_A0FIFO, AOFIFOdata, size, timeout, &r1));
			...
			/*FIFO DIO1*/
			size_t sizeDIO = FIFODEPTH2;
			NiFpga_Bool DOFIFOdata[FIFODEPTH2];
			DOFIFOdata[0] = 1;
			NiFpga_MergeStatus(&status, NiFpga_WriteU32(session, NiFpga_FPGA_ControlU32_DOIFIFOLoopPeriodus, DIOrate)); //rate
			NiFpga_MergeStatus(&status, NiFpga_WriteFifoBool(session, NiFpga_FPGA_HostToTargetFifoBool_DIOFIFO, DOFIFOdata, sizeDIO, timeout, &r2));
