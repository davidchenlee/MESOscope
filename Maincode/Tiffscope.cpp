...
void WriteFrameTiff(U32 *imageIn, std::string fileName)
{
	...
	unsigned char *image = new unsigned char[Ntotal_pix*sampleperpixel];
	...
	*/
	...
	//TAGS
	TIFFSetField(out, TIFFTAG_IMAGEWIDTH, Width_pix);					// set the width of the image
	TIFFSetField(out, TIFFTAG_IMAGELENGTH, Height_pix);					// set the height of the image
	...
	tsize_t linebytes = sampleperpixel * Width_pix;						// length in memory of one row of pixel in the image.
	...
	TIFFSetField(out, TIFFTAG_ROWSPERSTRIP, TIFFDefaultStripSize(out, Width_pix*sampleperpixel));

	//Now writing image to the file one strip at a time
	for (uint32 row = 0; row < Height_pix; row++)
	{
		memcpy(buf, &image[(Height_pix - row - 1)*linebytes], linebytes);    // check the index here, and figure out why not using h*linebytes
