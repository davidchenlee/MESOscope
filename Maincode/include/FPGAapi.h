...
namespace FPGAns
{
	...
	public:
		FPGA();
		~FPGA();
		void close(const bool reset = false) const;
	...
	public:
		...
		bool mStageAsTriggerEnable;																//Trigger the acquisition with the z stage: enable (0), disable (1)
		...
		RTsequence(const FPGAns::FPGA &fpga, const LineclockSelector lineclockInput = FG, const int nFrames = 1, const int widthPerFrame_pix = 300, const int heightPerFrame_pix = 400, const bool mStageAsTriggerEnable = false);
