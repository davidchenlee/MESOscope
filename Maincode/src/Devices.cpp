...
void Commandline::printToFile(std::ofstream *fileHandle) const
{
	...
	{
	case MOV:
		*fileHandle << actionToString_(mAction) << "\t" << mCommand.moveStage_.sliceNumber << "\t(" <<
			mCommand.moveStage_.stackIJ.at(XX) << "," << mCommand.moveStage_.stackIJ.at(YY) << ")\t(" <<
			std::fixed << std::setprecision(3) << mCommand.moveStage_.stackCenter_mm.at(0) << "," << mCommand.moveStage_.stackCenter_mm.at(1) << ")\n";
		break;
	case ACQ:
		scanZf_mm = mCommand.acqStack_.scanZi_mm + mCommand.acqStack_.scanDirZ * mCommand.acqStack_.stackDepth_um / 1000;
		scanPf_mW = mCommand.acqStack_.scanPi_mW + mCommand.acqStack_.scanDirZ * mCommand.acqStack_.stackPinc_mW;
		*fileHandle << actionToString_(mAction) << "\t\t\t\t\t" << mCommand.acqStack_.stackNumber << "\t" << mCommand.acqStack_.wavelength_nm << "\t" << mCommand.acqStack_.scanDirZ << "\t" <<
			std::setprecision(3) << mCommand.acqStack_.scanZi_mm << "\t" << scanZf_mm << "\t" <<
			std::setprecision(0) << mCommand.acqStack_.scanPi_mW << "\t" << scanPf_mW << "\n";
		break;
	case SAV:
	...
	case CUT:
		*fileHandle << actionToString_(mAction) << "*******************\tStage position (x,y,z) = (" <<
			std::setprecision(3) << mCommand.cutSlice_.stagePosition_mm.at(XX) << "," << mCommand.cutSlice_.stagePosition_mm.at(YY) << "," << mCommand.cutSlice_.stagePosition_mm.at(ZZ) << ") *******************************\n";
		break;
	default:
		throw std::invalid_argument((std::string)__FUNCTION__ + ": Selected action invalid");
	}
...
void Commandline::printParameters() const
{
	...
	case MOV:
		...
		std::cout << "Vibratome slice number = " << mCommand.moveStage_.sliceNumber << "\n";
		std::cout << "Stack ij = (" << mCommand.moveStage_.stackIJ.at(XX) << "," << mCommand.moveStage_.stackIJ.at(YY) << ")\n";
		std::cout << "Stack center (mm,mm) = (" << mCommand.moveStage_.stackCenter_mm.at(XX) << "," << mCommand.moveStage_.stackCenter_mm.at(YY) << ")\n" << std::endl;
		break;
	case ACQ:
		std::cout << "The command is " << actionToString_(mAction) << " with parameters: \n";
		std::cout << "wavelength (nm) = " << mCommand.acqStack_.wavelength_nm << "\n";
		std::cout << "scanDirZ = " << mCommand.acqStack_.scanDirZ << "\n";
		std::cout << "scanZi/stackDepth (mm) = " << mCommand.acqStack_.scanZi_mm << "/" << mCommand.acqStack_.stackDepth_um/1000 << "\n";
		std::cout << "scanPi/stackPdiff (mW) = " << mCommand.acqStack_.scanPi_mW << "/" << mCommand.acqStack_.stackPinc_mW << "\n" << std::endl;
		break;
	case SAV:
	...
	mSample(sample), mLaserConfig(laserConfig), mStackConfig(stackConfig), mScanZi_mm(stageInitialZ_mm), mVibratomeConfig(vibratomeConfig)
{
	...
	mStackArrayDim.at(XX) = static_cast<int>(std::ceil(mSample.mLength_um.at(XX) / mStackConfig.mFOV_um.at(XX)));					//Number of stacks in x
	mStackArrayDim.at(YY) = static_cast<int>(std::ceil(mSample.mLength_um.at(YY) / mStackConfig.mFOV_um.at(YY)));					//Number of stacks in y
...
double2 Sequencer::stackIndicesToStackCenter_mm_(const int2 stackArrayIndices) const
{
	...
	stagePosition_mm.at(XX) = mStackConfig.mFOV_um.at(XX)/1000 * (stackArrayIndices.at(XX) + 0.5);	// (stackIJ + 0.5) ranges from 0.5 to (mStackArrayDim - 0.5)
	stagePosition_mm.at(YY) = mStackConfig.mFOV_um.at(YY)/1000 * (stackArrayIndices.at(YY) + 0.5);	// (stackIJ + 0.5) ranges from 0.5 to (mStackArrayDim - 0.5)
...
void Sequencer::moveStage_(const int2 stackIJ)
{
	...
	commandline.mCommand.moveStage_ = { mSliceCounter, stackIJ, stackCenter_mm };
	mCommandList.push_back(commandline);
}

void Sequencer::acqStack_(const LaserConfig laserConfig)
{
	const double scanPi_mW = calculateStackScanInitialP_mW_(laserConfig.mScanPi_mW, laserConfig.mStackPinc_mW, mScanDir.at(ZZ));

	Commandline commandline;
	commandline.mAction = ACQ;
	commandline.mCommand.acqStack_ = { mStackCounter, laserConfig.mWavelength_nm, mScanDir.at(ZZ), mScanZi_mm, mStackConfig.mStackDepth_um, scanPi_mW, laserConfig.mStackPinc_mW };
	mCommandList.push_back(commandline);

	mStackCounter++;	//Keep track of the number of stacks acquired

	//Update the parameters for the next iteration
	mScanZi_mm += mScanDir.at(ZZ) * mStackConfig.mStackDepth_um / 1000;		//Next initial z-scan position
...
void Sequencer::cutSlice_()
{
	...
	commandline.mCommand.cutSlice_ = { mVibratomeConfig.mHomePosition_mm.at(XX), mVibratomeConfig.mHomePosition_mm.at(YY), mPlaneToCutZ_mm + mVibratomeConfig.mBladeOffsetZ_um/1000};
	mCommandList.push_back(commandline);
	...
	mPlaneToCutZ_mm += mStackConfig.mStackDepth_um / 1000;		//Increase the height of the cut plane for the next iteration
}
