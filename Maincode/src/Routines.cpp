...
namespace MainRoutines
{
	...
	void frameByFrameScan(const FPGAns::FPGA &fpga)
	{
		...
		//DATALOG
		{
			...
			datalog.record("Laser power increase (mW) = ", singleChannel.mStackPinc / mW);
...
//3. Tune Vision's wavelength manually
	...
	void demultiplexing(const FPGAns::FPGA &fpga)
	{
		...
		Galvo scan{ RTcontrol, RTSCANGALVO, FFOVslow / 2 };
		//Galvo scan{ RTcontrol, RTSCANGALVO, 0 };		//Keep the scanner at 0 to see the emitted light swing across the PMT16X channels
		Galvo rescan{ RTcontrol, RTRESCANGALVO, FFOVslow / 2 };
	...
	void PMT16Xconfig()
	{
		...
		pmt.setAllGain({
			...
			255,	//CH7
			255,	//CH8
			255,	//CH9, this channel presents the lowest signal. For some reason 255 gives a lower signal than 200
			255,	//CH10
			255,	//CH11
			255,	//CH12
			255,	//CH13
